...
func TestListNotes_FilterByTag(t *testing.T) {
	...
	listNotes(&buf, path, []string{"auth"}, 0)
	out := buf.String()
	if !strings.Contains(out, "fix login #auth") {
...
func TestListNotes_FilterByMultipleTags(t *testing.T) {
	...
	listNotes(&buf, path, []string{"auth", "backend"}, 0)
	out := buf.String()
...
func TestListNotes_NoFile(t *testing.T) {
	var buf bytes.Buffer
	listNotes(&buf, "/tmp/no-such-file-xyz.md", nil, 0)
	if !strings.Contains(buf.String(), "No notes yet") {
		t.Errorf("expected 'No notes yet', got: %q", buf.String())
	}
}
...
func TestListNotes_HidesTimestamps(t *testing.T) {
	...
	if err := listNotes(&buf, path, nil, 0); err != nil {
		t.Fatal(err)
	}
	out := buf.String()
...
func TestParseIDs_SpaceSeparated(t *testing.T) {
	ids, err := parseIDs([]string{"1", "2", "3"}, 10)
...
func TestParseIDs_Deduplicates(t *testing.T) {
	ids, err := parseIDs([]string{"1", "1", "2"}, 10)
...
func TestParseIDs_InvalidReturnsError(t *testing.T) {
	if _, err := parseIDs([]string{"abc"}, 10); err == nil {
		t.Error("expected error for non-integer id")
	}
}

func TestParseIDs_ClosedRange(t *testing.T) {
	ids, err := parseIDs([]string{"3..6"}, 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(ids) != 4 || ids[0] != 3 || ids[3] != 6 {
		t.Errorf("got %v, want [3 4 5 6]", ids)
	}
}

func TestParseIDs_OpenEndedRange(t *testing.T) {
	ids, err := parseIDs([]string{"8.."}, 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(ids) != 3 || ids[0] != 8 || ids[2] != 10 {
		t.Errorf("got %v, want [8 9 10]", ids)
	}
}

func TestParseIDs_MixedRangeAndSingle(t *testing.T) {
	ids, err := parseIDs([]string{"1..3", "5", "8.."}, 9)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseIDs_RangeDeduplicates(t *testing.T) {
	ids, err := parseIDs([]string{"1..3", "2,4"}, 10)
	if err != nil {
		t.Fatal(err)
	}
	// 1,2,3 from range + 2 (dup) + 4 = [1,2,3,4]
	if len(ids) != 4 {
		t.Errorf("got %v, want [1 2 3 4]", ids)
	}
}

func TestParseIDs_InvalidRangeReturnsError(t *testing.T) {
	if _, err := parseIDs([]string{"a..5"}, 10); err == nil {
		t.Error("expected error for invalid range start")
	}
	if _, err := parseIDs([]string{"1..z"}, 10); err == nil {
