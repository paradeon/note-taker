...
func listNotes(w io.Writer, file string, tags []string) error {
	if !hasContent(file) {
...
func main() {
	...
	default:
		...
		case "add":
			...
			err = appendNote(os.Stdout, file, processTags(strings.Join(contentArgs[1:], " ")))
		case "list":
			var filterTags []string
			if flagTag != "" {
				filterTags = strings.Split(flagTag, ",")
			}
			err = listNotes(os.Stdout, file, filterTags)
		case "tags":
